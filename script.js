...
function isDPSale(item) {
  const text = [item.display, item.price, item.description, item.note].join(" ").toUpperCase();

  return text.includes("DP SALE") ||
...
function setActiveButton(clickedButton) {
  floorButtons.forEach((button) => button.classList.remove("is-active"));
